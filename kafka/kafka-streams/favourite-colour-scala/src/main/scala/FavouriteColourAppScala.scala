package com.github.muhdlaziem.kafka.streams
...
object FavouriteColourAppScala {
  def main(args: Array[String]): Unit = {
    ...
    config.put(StreamsConfig.APPLICATION_ID_CONFIG, "favourite-colour-scala1")
    config.put(StreamsConfig.BOOTSTRAP_SERVERS_CONFIG, "127.0.0.1:9092")
    config.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest")
    config.put(StreamsConfig.DEFAULT_KEY_SERDE_CLASS_CONFIG, Serdes.String.getClass)
    config.put(StreamsConfig.DEFAULT_VALUE_SERDE_CLASS_CONFIG, Serdes.String.getClass)
    config.put(StreamsConfig.CACHE_MAX_BYTES_BUFFERING_CONFIG, 0L)
...
//    config.put(StreamsConfig.CACHE_MAX_BYTES_BUFFERING_CONFIG, "0")
    ...
    val intermediaryTopic = "user-keys-and-colours"
